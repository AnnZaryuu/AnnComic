            ...
            <a href="/index.php?modul=profile">
                <button class="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-3xl hover:bg-blue-700">
                    <div class="w-8 h-8 rounded-full overflow-hidden bg-gray-200">
                        <img src="<?= htmlspecialchars($_SESSION['profile_picture'] ?? $user->profilePicture ?? '') ?>" alt="Profile Picture" class="w-full h-full object-cover" />
                    </div>
                    <span><?= htmlspecialchars($_SESSION['username'] ?? $user->name ?? '') ?></span>
                </button>
            </a>
        </div>
    </div>
